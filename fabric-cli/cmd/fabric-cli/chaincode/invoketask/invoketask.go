...
func New(id string, channelClient apitxn.ChannelClient, ccID string, args *action.ArgStruct, targets []apifabclient.Peer,
	executor *executor.Executor, maxAttempts int, resubmitDelay time.Duration, verbose bool, p printer.Printer, callback func(err error)) *Task {
	return &Task{
		id:            id,
...
func (t *Task) doInvoke() error {
	cliconfig.Config().Logger().Debugf("(%s) - Invoking chaincode: %s, function: %s, args: %v, on %s. Attempt #%d...\n",
		t.id, t.ccID, t.args.Func, t.args.Args, asString(t.targets), t.attempt)
