...
func TestInstall_chaincode_on_all_peers_of_org1(t *testing.T) {
	...
	run("fabric-cli.go chaincode install --cid=mychannel --orgid org1 --ccp=github.com/example_cc --ccid=ExampleCC --v v0 --gopath=../fixtures/testgopath --config ../fixtures/config/config_test.yaml")
}

func TestInstall_chaincode_on_all_peers_of_org2(t *testing.T) {
	header("Install chaincode on all peers of org2")
	run("fabric-cli.go chaincode install --cid=mychannel --orgid org2 --ccp=github.com/example_cc --ccid=ExampleCC --v v0 --gopath=../fixtures/testgopath --config ../fixtures/config/config_test.yaml")
}

func TestInstall_chaincode_on_all_peers(t *testing.T) {
	header("Install chaincode on all peers")
	run("fabric-cli.go chaincode install --cid=mychannel --ccp=github.com/example_cc --ccid=ExampleCC --v v0 --gopath=../fixtures/testgopath --config ../fixtures/config/config_test.yaml")
}
...
func TestQuery_info(t *testing.T) {
	header("Query info")
	run("fabric-cli.go query info --cid mychannel --config ../fixtures/config/config_test.yaml")
}

func TestQuery_block_by_block_number(t *testing.T) {
	header("Query block by block number")
	run("fabric-cli.go query block --cid mychannel --num 0 --config ../fixtures/config/config_test.yaml")
}

func TestQuery_block_by_hash(t *testing.T) {
	header("Query block by hash")
	// hash is the output of "query info"
	run("fabric-cli.go query block --cid mychannel --hash qhjKLn5VrXs66p0-92D7Boqswl7UjK6FjiHVmzF0z_0 --config ../fixtures/config/config_test.yaml")
}

func TestQuery_block_output_in_JSON_format(t *testing.T) {
	header("Query block output in JSON format")
	run("fabric-cli.go query block --cid mychannel --num 0 --format json --config ../fixtures/config/config_test.yaml")
}

// TODO
func TestQuery_transaction(t *testing.T) {
	header("Query transaction")
	run("fabric-cli.go query tx --cid mychannel --txid 29bd4fd03e657da488acfa8ae1740eebf4a6ee81399bfc501f192cb407d2328c --config ../fixtures/config/config_test.yaml")
}

func TestQuery_channels_joined_by_a_peer(t *testing.T) {
	header("Query channels joined by a peer")
	run("fabric-cli.go query channels --peer localhost:7051 --config ../fixtures/config/config_test.yaml")
}

func TestQuery_installed_chaincodes_on_a_peer(t *testing.T) {
	header("Query installed chaincodes on a peer")
	run("fabric-cli.go query installed --peer localhost:7051 --config ../fixtures/config/config_test.yaml")
}
