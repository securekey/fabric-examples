...
func (p *printer) PrintBlockchainInfo(info *fabricCommon.BlockchainInfo) {
	...
	p.PrintHeader()
	p.print("Block Info for chain %s", ChannelID)
...
func (p *printer) PrintChaincodeData(ccData *ccprovider.ChaincodeData) {
	...
	depSpec := &fabricPeer.ChaincodeDeploymentSpec{}
	unmarshalOrPanic(ccData.DepSpec, depSpec)
	p.element("DepSpec")
	p.printChaincodeDeploymentSpec(depSpec)
	p.elementEnd()
	p.PrintFooter()
}
...
func (p *printer) printChaincodeAction(chaincodeAction *peer.ChaincodeAction) {
	...
	if err := txRWSet.Unmarshal(chaincodeAction.Results); err != nil {
		panic(err)
	}
...
func (p *printer) printTxReadWriteSet(txRWSet *rwset.TxReadWriteSet) {
	p.array("NsRWs")
	for i, nsRWSet := range txRWSet.NsRWs {
		p.item("TxReadWriteSet", i)
...
func (p *printer) printNsReadWriteSet(nsRWSet *rwset.NsReadWriteSet) {
	p.field("NameSpace", nsRWSet.NameSpace)
	p.array("RangeQueriesInfo")
	for i, rqi := range nsRWSet.RangeQueriesInfo {
...
func (p *printer) printRangeQueryInfo(rqi *rwset.RangeQueryInfo) {
	p.field("TODO", nil)
}

func (p *printer) printRead(r *rwset.KVRead) {
	p.field("Key", r.Key)
	p.element("Version")
...
func (p *printer) printVersion(version *version.Height) {
	if version == nil {
		return
	}
...
func (p *printer) printWrite(w *rwset.KVWrite) {
	p.field("Key", w.Key)
...
func (p *printer) printConfigGroup(configGroup *fabricCommon.ConfigGroup) {
	...
	for key, policy := range configGroup.Policies {
		p.item("Policy", key)
		p.printConfigPolicy(key, policy)
		p.itemEnd()
	}
	p.arrayEnd()
}

func (p *printer) printConfigPolicy(name string, policy *fabricCommon.ConfigPolicy) {
	...
	case fabricCommon.Policy_SIGNATURE:
		...
		unmarshalOrPanic(policy.Policy.Policy, sigPolicyEnv)
	...
	case fabricCommon.Policy_IMPLICIT_META:
		...
		unmarshalOrPanic(policy.Policy.Policy, impMetaPolicy)
...
func (p *printer) printSignaturePolicyEnvelope(sigPolicyEnv *fabricCommon.SignaturePolicyEnvelope) {
	p.element("Policy")
	p.printSignaturePolicy(sigPolicyEnv.Policy)
	p.elementEnd()
...
func (p *printer) printSignaturePolicyNOutOf(sigPolicy *fabricCommon.SignaturePolicy_NOutOf) {
	...
	p.array("Policies")
	for i, policy := range sigPolicy.Policies {
		p.item("Policy", i)
...
func (p *printer) printConfigValue(name string, value *fabricCommon.ConfigValue) {
	...
	case configtx.CreationPolicyKey:
		...
		p.print("Creation Policy:")
		p.field("Policy", creationPolicy.Policy)
		break
	...
	case fabricConfig.BlockDataHashingStructureKey:
		...
		p.print("Block Data Hashing Structure:")
