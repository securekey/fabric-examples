...
func buildChaincodePolicy_orig(mspid string) (*common.SignaturePolicyEnvelope, error) {
	// Define MSPRole
...
func buildChaincodePolicy(mspid string) (*common.SignaturePolicyEnvelope, error) {
	member1Role, err := proto.Marshal(&common.MSPRole{Role: common.MSPRole_MEMBER, MspIdentifier: "Org1MSP"})
	if err != nil {
		return nil, fmt.Errorf("Error marshal MSPRole: %s", err)
	}
	princ1 := &common.MSPPrincipal{
		PrincipalClassification: common.MSPPrincipal_ROLE,
		Principal:               member1Role}

	member2Role, err := proto.Marshal(&common.MSPRole{Role: common.MSPRole_ADMIN, MspIdentifier: "Org2MSP"})
	if err != nil {
		return nil, fmt.Errorf("Error marshal MSPRole: %s", err)
	}
	princ2 := &common.MSPPrincipal{
		PrincipalClassification: common.MSPPrincipal_ROLE,
		Principal:               member2Role}

	member3Role, err := proto.Marshal(&common.MSPRole{Role: common.MSPRole_MEMBER, MspIdentifier: "Org3MSP"})
	if err != nil {
		return nil, fmt.Errorf("Error marshal MSPRole: %s", err)
	}
	princ3 := &common.MSPPrincipal{
		PrincipalClassification: common.MSPPrincipal_ROLE,
		Principal:               member3Role}

	member4Role, err := proto.Marshal(&common.MSPRole{Role: common.MSPRole_MEMBER, MspIdentifier: "Org4MSP"})
	if err != nil {
		return nil, fmt.Errorf("Error marshal MSPRole: %s", err)
	}
	princ4 := &common.MSPPrincipal{
		PrincipalClassification: common.MSPPrincipal_ROLE,
		Principal:               member4Role}

	signedBy1 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_SignedBy{
			SignedBy: 0,
		}}
	signedBy2 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_SignedBy{
			SignedBy: 1,
		}}
	signedBy3 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_SignedBy{
			SignedBy: 2,
		}}
	signedBy4 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_SignedBy{
			SignedBy: 3,
		}}

	// construct 'one of one' policy
	policy_1_and_2 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_NOutOf_{
			NOutOf: &common.SignaturePolicy_NOutOf{
				N:        2,
				Policies: []*common.SignaturePolicy{signedBy1, signedBy2}}}}
	policy_2_of_1_3_4 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_NOutOf_{
			NOutOf: &common.SignaturePolicy_NOutOf{
				N:        2,
				Policies: []*common.SignaturePolicy{signedBy1, signedBy3, signedBy4}}}}
	policy_1_and_2_or_2_of_1_3_4 := &common.SignaturePolicy{
		Type: &common.SignaturePolicy_NOutOf_{
			NOutOf: &common.SignaturePolicy_NOutOf{
				N:        1,
				Policies: []*common.SignaturePolicy{policy_1_and_2, policy_2_of_1_3_4}}}}

	p := &common.SignaturePolicyEnvelope{
		Version:    0,
		Policy:     policy_1_and_2_or_2_of_1_3_4,
		Identities: []*common.MSPPrincipal{princ1, princ2, princ3, princ4},
	}
	return p, nil
}
